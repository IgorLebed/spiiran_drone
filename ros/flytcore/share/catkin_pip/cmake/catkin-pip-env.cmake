...
function(catkin_pip_check_env config_prefix)
    ...
    string(FIND "$ENV{PYTHONPATH}" ${config_prefix}/lib/python2.7/site-packages FOUND_PPPATH)
    string(FIND "$ENV{PYTHONPATH}" ${CATKIN_DEVEL_PREFIX}/lib/python2.7/site-packages FOUND_WPPPATH)
    string(FIND "$ENV{PATH}" ${config_prefix}/bin FOUND_SPATH)
    ...
    if ( ( FOUND_PPATH LESS 0 ) OR ( FOUND_PPPATH LESS 0 ) OR ( FOUND_SPATH LESS 0 ) )
        ...
        if ( FOUND_PPPATH LESS 0 )
            message(STATUS "Adding ${config_prefix}/lib/python2.7/site-packages to PYTHONPATH")
            set(ENV{PYTHONPATH} "${config_prefix}/lib/python2.7/site-packages:$ENV{PYTHONPATH}")
        endif()
        ...
        if ( FOUND_WPPPATH LESS 0 )
            message(STATUS "Adding ${CATKIN_DEVEL_PREFIX}/lib/python2.7/site-packages to PYTHONPATH")
            set(ENV{PYTHONPATH} "${CATKIN_DEVEL_PREFIX}/lib/python2.7/site-packages:$ENV{PYTHONPATH}")
        endif()

        if ( FOUND_SPATH LESS 0 )
            message(STATUS "Adding ${config_prefix}/bin to PATH")
            set(ENV{PATH} "${config_prefix}/bin:$ENV{PATH}")
        endif()
