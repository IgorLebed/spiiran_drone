...
function(catkin_pip_setup_prefix ws_prefix)
    ...
    find_program(CATKIN_PIP NAMES pip pip2 pip2.7 PATHS ${ws_prefix}/bin NO_DEFAULT_PATH)
    ...
    if (CATKIN_PIP)
        ...
        unset(CATKIN_PIP CACHE)
        message(STATUS "    ... Catkin pip was not found in ${ws_prefix}/bin ...")
        ...
        if (${CATKIN_SYS_PIP_VERSION} VERSION_LESS 6.0.0)
            ...
            execute_process(
              COMMAND ${CATKIN_SYS_PIP} -q install ${OPT_SYSTEM} -r "${CATKIN_PIP_REQUIREMENTS_PATH}/catkin-pip-base.req" --download-cache "${CMAKE_BINARY_DIR}/pip-cache" --target "${ws_prefix}/lib/python2.7/site-packages" --exists-action w
              WORKING_DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR}
            ...
            if (PIP_RESULT)
                message(STATUS "Command ${CATKIN_SYS_PIP} install ${OPT_SYSTEM} -r \"${CATKIN_PIP_REQUIREMENTS_PATH}/catkin-pip-base.req\" --download-cache \"${CMAKE_BINARY_DIR}/pip-cache\" --target \"${ws_prefix}/lib/python2.7/site-packages\" --exists-action w FAILED")
            ...
            execute_process(
              COMMAND ${CATKIN_SYS_PIP} -q install ${OPT_SYSTEM} -r "${CATKIN_PIP_REQUIREMENTS_PATH}/catkin-pip-base.req" --cache-dir "${CMAKE_BINARY_DIR}/pip-cache" --target "${ws_prefix}/lib/python2.7/site-packages" --exists-action w
              WORKING_DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR}
              RESULT_VARIABLE PIP_RESULT
              OUTPUT_VARIABLE PIP_VARIABLE
              ERROR_VARIABLE PIP_ERROR
            ...
            message(STATUS "    ... Done ... [${PIP_RESULT}]: ${PIP_VARIABLE}")
            if (PIP_RESULT)
                message(STATUS "Command ${CATKIN_SYS_PIP} install ${OPT_SYSTEM} -r \"${CATKIN_PIP_REQUIREMENTS_PATH}/catkin-pip-base.req\" --cache-dir \"${CMAKE_BINARY_DIR}/pip-cache\" --target \"${ws_prefix}/lib/python2.7/site-packages\" --exists-action w FAILED")
                message(FATAL_ERROR "${PIP_ERROR}")
            endif()
        endif()
        ...
        if (CATKIN_PIP)
            ...
            message( FATAL_ERROR "catkin_pip pip command not found in ${ws_prefix}/bin. Make sure you have installed the pip pip package on ${ws_prefix} workspace.")
        endif()
