...
while RUN:
    ...
    if make_photo(last_time, now):
        ...
        lo = pose_v.lon
        al = pose_v.alt
        img_set = pexif.JpegFile.fromFile("/flyt/userapps/onboard_user/install/flight_to_point_and_photo/photo/" +  str(img_counter) + ".jpg")
        img_set.set_geo(float(la), float(lo), float(al))
        img_set.writeFile("/flyt/userapps/onboard_user/install/flight_to_point_and_photo/photo_with_geo/" + str(img_counter) + "_with_geo" + ".jpg")
        img_counter += 1
        last_time = now
