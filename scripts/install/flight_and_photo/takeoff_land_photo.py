...
while RUN:
    ...
    if make_photo(last_time, now):
        ...
        lo = pose_v.lon
        al = pose_v.alt
        img_set = pexif.JpegFile.fromFile(str(img_counter) + ".jpg")
        img_set.set_geo(float(la), float(lo), float(al))
        #path = '/flyt/userapps/onboard/install/flight_and_photo/photo'
        img_set.writeFile(str(img_counter) + "_with_geo" + ".jpg")
        img_counter += 1
        last_time = now
